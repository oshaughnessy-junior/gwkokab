...
def _discrete_group(d, sig, key, M=20_000):
    ...
    pe = jnp.stack([
        d[i] + sig[i] * jax.random.normal(keys[i], (M, 1)) for i in range(n)
    ])
...
def test_MX1_discrete_only_matches_exact():
    ...
    ll_m, _ = mixed_poisson_likelihood_fn(
        ...
        samples,
        log_w,
        None,  # empty sampled group (samples kept for μ)
        model,
        dg,
        lpg,
        mg,
...
def test_MX2_split_equals_whole_and_exact():
    ...
    ll_all, _ = sampled_poisson_likelihood_fn(
        _event_Ls(), samples, log_w, None, _T, None
    )
    ll_exact = _exact_logL(_D[:, 0], _SIGMA, _LOGR, _M, _S, _T)
...
def test_MX3_diagnostics_present():
    ...
    model = _scaled_mvn(_LOGR, _M, _S)
    dg, lpg, mg, npes = _discrete_group(
        _D[:3], _SIGMA[:3], jax.random.PRNGKey(6), M=5_000
    )
    _, diag = mixed_poisson_likelihood_fn(
        _event_Ls(_D[3:], _SIGMA[3:]),
        samples,
        log_w,
        None,
        model,
    ...
    for k in (
        "ess_per_event_sampled",
        "log_evidence_per_event_sampled",
        "log_evidence_per_event_discrete",
        "expected_rate",
    ):
        assert k in diag
